		...
		if (opt1.isEmpty()) {
			...
			while(optionLoop){
				...
				case 0:
					...
					System.out.println("Player "+ gamePlayer.getPlayerNumber() + " get "+ returnedColor+" color.");
				...
				case 1:
					...
					if (checkPossibleUniqueColor(gamePlayer) ){ //|| (sevenValue > sixValue && sevenValue > fourValue)
						...
						if(gamePlayer.getPossibleDedicationSevenUniqueColor().size() > 6)
						{				
							if (checkPossibleUniqueColor(gamePlayer)) {

								doDedicationSeven(gameObj, gamePlayer);
							}
						}else if(gamePlayer.getPossibleDedicationThreePairColor().size() > 3)
						{
						...
						}else if(gamePlayer.getPossibleDedicationFourUniqueColor().size() > 0){
					...
					System.out.println("Player " + gamePlayer.getPlayerNumber() + " score: "+ gamePlayer.getTotalPoints());
...
//					otherPlayerTilesPlacement  += lakeTileChoice.getTilesId() +":"+ degree;
			...
			for (int i = 0; i < tilePlacementCominationResults.size(); i++) {
				...
				if(Integer.parseInt(result[3])+Integer.parseInt(result[5])+Integer.parseInt(result[7])+Integer.parseInt(result[9])>3)
				{
...
//					playing = removePlacedLakeTile(tileInHand, playing);
			...
			Players playing, String option) 
	{
		...
		while (playing.getLanternCardCount() > 12) 
		{
			...
			if (option.contains("type1") || option.contains("type2")
					|| option.contains("type3")) {
				System.out.println("1: Make a dedication.");
			}
