...
/**
 * This class is used for creating objects of FriendlyPlayer strategy. Friedly
 * player is one of the four strategies implemented for build 3
 * 
 * @author Team E
 * @Since 8/8/15
 * 
...
public class FriendlyPlayerStrategy extends PlayerStrategy {
	 ...
	 * 
	 * @param gameObj
	 *            is the instance of GameInstance class
	 */
	...
	/**
	 * This method is used to store the logic of last turn of the player when
	 * the game is about to end
	 * 
	 * @param gameobj
	 *            object of GameInstance Class
	 * @param gamePlayer
	 *            object of Players class
	 * @param opt1
	 *            is used to check whether exchange has been done beforee
	 */
			...
			while (optionLoop) {
				...
				case 0:
					...
					if (gamePlayer.getPlayerFavorToken() > 1) {
						...
						// get the empty color
						doExchange(gamePlayer, gameObj, getEmptyColor);
				...
				case 1:
					...
					if (checkPossibleUniqueColor(gamePlayer)) { // ||
																...
																// sevenValue >
																// fourValue)
						// || (genericValue > sixValue && genericValue >
						// fourValue
					...
					} else if ((colorPairs.size() > 2)) {// || (sixValue >
															// sevenValue &&
															// sixValue >
															// fourValue)) {
						 ...
						 * else if (genericValue > sixValue && genericValue >
						 * fourValue && genericValue > sevenValue) {
						 * 
						 * //doWhatIsAsked(playing, gameObjs, "GEN");
						 * 
						 * // If anyone of the dedication is possible, do it.
						 * if(gamePlayer.getPossibleDedicationSevenUniqueColor()
						 * .size() > 6) { if
						 * (checkPossibleUniqueColor(gamePlayer)) {
						 * 
						 * doDedicationSeven(gameObj, gamePlayer); } }else
						 * if(gamePlayer.getPossibleDedicationThreePairColor().
						 * size() > 3) {
						 * 
						 ...
						 * 
						 * }else
						 * if(gamePlayer.getPossibleDedicationFourUniqueColor().
						 * size() > 0){
						 * 
						 * if(fourUniqueColorPairs.size() > 0) {
						 * 
						 * int randomNumber =
						 * getRandomNumber(fourUniqueColorPairs.size());
						 * 
						 * String colorFour =
						 * fourUniqueColorPairs.get(randomNumber);
						 * 
						 * doDedicationFour(gameObj, colorFour, gamePlayer); }
						 * 
						 * } }
				...
				// }
				/*
				 * if (optionTurnCheck == 1) { if (lastExchange == 1 ||
				 * lastDedication == 1) { optionLoop = false; } }
				 */
				// optionTurnCheck = 0;
			}
		}
	...
	public String makeAMove(Scanner scan, GameInstance gameObjs, Players playing, String option) {
		...
		if (playing.getLanternCardCount() > 12) {
			...
			// Map<String,Integer> playerMatchingCardColors = new
			...
			for (int i = 0; i < placedTileCombinations.size(); i++) {
				...
				/*
				 * if(lakeTileChoice.isPlatform()){ playerFavorToken =
				 * playerFavorToken+1; }
				 */
				...
				for (int plyrCount = 0; plyrCount < gameObjs.getPlayersList().length; plyrCount++) {
					...
					// otherPlayerTilesPlacement += lakeTileChoice.getTilesId()
					// +":"+ degree;
				}
			...
			/*
			 * if(tileMove.equalsIgnoreCase("")) { //random placing any tile }
			 * else { //place lakeTile }
			 */
			...
			// LakeTiles tileInHand = placeALakeTileAndDistribute(gameObjs,
			// playing, tileMove);
	 ...
	 *            instance of GameInstance class
	 * @param playing
	 *            instance of Players class
	 * @param combination
	 *            of results for adjacent tile position
	 * @param adjTilePosition
	 *            String to check adjacent tile position
	...
	public static LakeTiles placeALakeTileAndDistribute(GameInstance gameObjs, Players playing, String comRes,
		...
		// LakeTiles tileInHand =
		// playing.getCurrentLakeTilesHold().get(Integer.parseInt(placingInformation[0]));
	 ...
	 *            the laketile which is placed on the board
	 ...
	 *            object of GameInstance class
	 * @param type
	 *            is used to check the eligibility for performing a certain type
	 *            of dedication
	 */
	 ...
	 *            object of Players class
	 * @param giveColor
	 *            gives the lantern card colors for performing certain type of
	 *            dedication
	 */
