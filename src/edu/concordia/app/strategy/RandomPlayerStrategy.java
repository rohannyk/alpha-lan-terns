...
public class RandomPlayerStrategy extends PlayerStrategy implements Serializable {
	...
	public String makeAMove(GameInstance gameObjs, Players playing, String opt) {
		...
		if ((opt.contains("exchange") && !(opt.contains("type1")
				|| opt.contains("type2") || opt.contains("type3"))) || (!opt.contains("exchange") && (opt.contains("type1")
						|| opt.contains("type2") || opt.contains("type3")))) {
		...
		if(opt.contains("exchange") && (opt.contains("type1") || opt.contains("type2")
				|| opt.contains("type3"))){
...
*/
	...
	public boolean placeALakeTileAndDistribute(GameInstance gameObjs, Players playing, String placementString, LakeTiles tileInHand)
	{
		...
		/*if(resultChoice.length == (positionChoice+1)){
			
		}else if(resultChoice.length < (positionChoice+1)){
			positionChoice = positionChoice +1;
		}*/
	...
	public void exchageLaternCard(GameInstance gameObj, Players playing) {
		...
		String gameColor = "";
			...
			System.out.println("Number of your Favor "
					+ "tokens present currently: = " + playing.getPlayerFavorToken());
			System.out.println();
...
//			loop = playing.lanternColorPlayerValidation(yourColor);
...
//			loop = playGame.lanternColorGameValidation(gameObj, gameColor);
	...
	public String filterExcessLanternCards(GameInstance gameObj,
		...
		while (playing.getLanternCardCount() > 12) 
		{
			...
			case 2:
				//System.out.println(playing.getPlayerNumber()+" hello");
	...
	public void makeADedication(GameInstance gameObj, String opt, Players playing) {
			...
			case 1:
				if (opt.contains("type1")) {
					...
					boolean type1Val = PlayGame
							.getDedicationType1ColorValidationAndRemoval(playing,
									color1, gameObj);
					if (type1Val) {
			...
			case 2:
				if (opt.contains("type2")) {
					...
					System.out
					.println("Input second color of lantern card: "+ type2ColorOptions.get(type2Color));
					...
					System.out
					.println("Input third color of lantern card "+ type2ColorOptions.get(type2Color));
